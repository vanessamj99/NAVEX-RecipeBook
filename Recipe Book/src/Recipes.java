...
public class Recipes {
    ...
    public boolean checkRepeat(String recipeTitle) {
        for(int j = 0; j < RecipeBook.recipes.size(); j++){
            ...
            if (RecipeBook.recipes.get(j).title.equalsIgnoreCase(recipeTitle)) return true;
        }
        return false;
    }
    ...
    public void retrieve(String word){
        ...
        for(int j = 0; j < RecipeBook.recipes.size(); j++){
            // if recipe match is found, print out the details of the recipe
            if (RecipeBook.recipes.get(j).title.equalsIgnoreCase(word)){
        ...
        if(flag == -1) {
        	...
        	if(flag == 0) {
                ...
                for(int i = 0; i < RecipeBook.recipes.size(); i++){
                    // if recipe match is found, print out the details of the recipe
                    if (RecipeBook.recipes.get(i).title.equalsIgnoreCase(findWo)){
                        flag = 1;
...
//        		}
        		int distance =  StringUtils.getLevenshteinDistance(word, RecipeBook.recipes.get(i).title.toLowerCase());
        		if(distance <= 1.5) {
        			flag = 2;
    ...
    public void explore(RecipesDetails recipe) {
        ...
        if(option == 'W' || option == 'w') {
            ...
            for(int s = 0; s < steps.length; s++) {
            	...
            	if(!steps[s].equalsIgnoreCase("")) System.out.println(steps[s]);
            }
        }
            ...
            for(int s = 0; s < steps.length; s++) {
                ...
                for(int i = 0; i < parseStps.length; i++){
                	if(!parseStps[i].equalsIgnoreCase("")) {
